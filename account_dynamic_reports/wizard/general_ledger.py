...
class InsGeneralLedger(models.TransientModel):
    ...
    analytic_tag_ids = fields.Many2many(
        'account.analytic.plan', string='Analytic Plans'
    )
    ...
    def process_filters(self):
        ...
        if data.get('analytic_tag_ids', []):
           filters['analytic_tags'] = self.env['account.analytic.plan'].sudo().browse(data.get('analytic_tag_ids', [])).mapped('name')
        else:
    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            if data.get('analytic_ids',[]):
               analytic_account_ids = [[str(account_id) for account_id in data.get('analytic_ids',[])]]
               WHERE += ' AND "l".analytic_distribution ?| array[%s]' % str(analytic_account_ids)

            if data.get('analytic_tag_ids', []):
                analytic_tag_ids=self.env['account.analytic.plan'].browse(data.get('analytic_tag_ids', []))
                account_ids=[]
                for plan in analytic_tag_ids:
                    account_ids+=plan.account_ids.ids
                analytic_account_ids = [[str(account_id) for account_id in account_ids]]
                WHERE += ' AND "l".analytic_distribution ?| array[%s]' % str(analytic_account_ids)
    ...
    def get_filters(self, default_filters={}):
        ...
        analytic_tags = self.analytic_tag_ids if self.analytic_tag_ids else self.env['account.analytic.plan'].sudo().search(
