odoo.define('account_financial_reports_knk.general_ledger_report', function (require) {
'use strict';
...
var GeneralLedgerReport = stock_report_generic.extend({
    events: {
        'click .o_account_report_unfoldable': '_onClickUnfold',
        'click .o_account_report_foldable': '_onClickFold',
        'click .js_account_move_line_footnote': '_onClickShowMove',
    },
    init: function(){
    ...
    get_html: function() {
        ...
        return this._rpc({
                model: 'report.account_financial_reports_knk.report_general_ledger',
                method: 'get_html',
                args: args,
    ...
    renderSearch: function () {
        ...
        this.$searchView.find('.js_foldable_trigger').on('click', this._onClickFoldable.bind(this));
        this.$searchView.find('.js_account_report_date_cmp_filter').on('click', this._onDateCMP.bind(this));
    },
    ...
    _onClickPrint: function (ev) {
        framework.blockUI();
        var reportname = 'account_financial_reports_knk.report_general_ledger?docids=' + 1 +
    ...
    _onClickPrint_xls: function(ev) {
            ...
            .then(function (result) {
                session.get_file({
                    url: '/web/general_ledger_report/export',
...
core.action_registry.add('general_ledger_report', GeneralLedgerReport);
