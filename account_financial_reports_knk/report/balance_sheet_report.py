...
class ReportBalanceSheet(models.AbstractModel):
    _name = 'report.account_financial_reports_knk.report_balance_sheet'
    ...
    def _get_report_vals(self, dates, entry_type, journal_id, company_id, prev_period, last_period):
        ...
        total_curr_assets = 0.0
        total_prepayments = 0.0
        total_curr_lia = 0.0
        total_payables = 0.0
        total_non_curr_lia = 0.0
        total_curr_earning = 0.0
        total_curr_alloc_earning = 0.0
        total_retained_earning = 0.0
        for ln in line_data:
            ...
            if account.account_type == 'asset_cash':
                bankcash_lines.append(vals)
                total_bankcash += ln['debit'] - ln['credit']
            elif account.account_type == 'asset_receivable':
                rece_lines.append(vals)
                total_receivable += ln['debit'] - ln['credit']
            elif account.account_type == 'asset_current':
                curr_assets.append(vals)
                total_curr_assets += ln['debit'] - ln['credit']
            elif account.account_type == 'asset_prepayments':
                prepayments.append(vals)
                total_prepayments += ln['debit'] - ln['credit']
            elif account.account_type in ['liability_credit_card', 'liability_current']:
                curr_lia.append(vals)
                total_curr_lia += ln['debit'] - ln['credit']
            elif account.account_type == 'liability_payable':
                payables.append(vals)
                total_payables += ln['debit'] - ln['credit']
            elif account.account_type == 'liability_non_current':
                non_curr_lia.append(vals)
                total_non_curr_lia += ln['debit'] - ln['credit']
                    ...
                    'expense_direct_cost']:
                curr_earning.append(vals)
                total_curr_earning += ln['debit'] - ln['credit']
            elif account.account_type == self.env.ref('account.data_unaffected_earnings').id:
                curr_alloc_earning.append(vals)
                total_curr_alloc_earning += ln['debit'] - ln['credit']
            elif account.account_type == self.env.ref('account.data_account_type_equity').id:
                retained_earning.append(vals)
                total_retained_earning += ln['debit'] - ln['credit']
        currency_id = self.env.company.currency_id
        return {
            'bankcash_lines': bankcash_lines,
            'rece_lines': rece_lines,
            'curr_assets': curr_assets,
            'prepayments': prepayments,
            'curr_lia': curr_lia,
            'payables': payables,
            'non_curr_lia': non_curr_lia,
            'curr_earning': curr_earning,
            'curr_alloc_earning': curr_alloc_earning,
            'retained_earning': retained_earning,
            'total_bankcash': total_bankcash,
            'total_receivable': total_receivable,
            'total_curr_assets': total_curr_assets,
            'total_prepayments': total_prepayments,
            'total_curr_lia': total_curr_lia,
            'total_payables': total_payables,
            'total_non_curr_lia': total_non_curr_lia,
            'total_curr_earning': total_curr_earning,
            'total_retained_earning': total_retained_earning,
            'total_curr_alloc_earning': total_curr_alloc_earning,
            'string': string,
    ...
    def get_html(self, dates='this_month', entry_type='all', journal_id=False, company_id=False, prev_period=False, last_period=False):
        ...
        # res['lines'] = self.env.ref('account_financial_reports_knk.report_balance_sheet_document')._render({'data': vals})
        res['lines'] = self.env['ir.ui.view']._render_template('account_financial_reports_knk.report_balance_sheet_document', {'data': vals})
        res['selected_date_bs'] = dates
        return res
