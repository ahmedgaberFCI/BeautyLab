...
class ReportTrialBalance(models.AbstractModel):
    _name = 'report.account_financial_reports_knk.report_trial_balance'
    ...
    def _get_inital_balance(self, date_from, account, entry_type, journal_id, company_id):
        ...
        domain += [('date', '<', date_from), ('account_id', '=', account.id)]
        ...
        if last_period:
            ...
            while last_period > 0:
                ...
                if dates is None or dates == 'this_month':
                    ...
                    dt_from = datetime.strptime(self.env.context.get('from_date'), '%Y-%m-%d')
                    dt_to = datetime.strptime(self.env.context.get('to_date'), '%Y-%m-%d')
                    domain = self._get_domain(account, dates, entry_type, journal_id, company_id)
                    domain += [('date', '>=', dt_from), ('date', '<=', dt_to)]
                    string = "From " + self.env.context.get('from_date') + " To " + self.env.context.get('to_date')
                    compare_period_names.append({'name': string})
                line_data = self.env['account.move.line'].read_group(domain, ['account_id', 'debit', 'credit'], ['account_id'])
    ...
    def _get_report_vals(self, dates, entry_type, journal_id, company_id, prev_period, last_period):
        ...
        if dates == 'this_month':
            ...
            date_from = datetime.strptime(self.env.context.get('from_date'), '%Y-%m-%d')
            date_to = datetime.strptime(self.env.context.get('to_date'), '%Y-%m-%d')
            domain += [('date', '>=', date_from), ('date', '<=', date_to)]
            string = "From " + self.env.context.get('from_date') + " To " + self.env.context.get('to_date')
        line_data = self.env['account.move.line'].read_group(domain, ['account_id', 'debit', 'credit'], ['account_id'])
        for ln in line_data:
            account = self.env['account.account'].browse(ln['account_id'][0])
            inital_balance = self._get_inital_balance(date_from, account, entry_type, journal_id, company_id)
            ...
            move_lines.append({
                ...
                'init_debit': inital_balance['debit'],
                'init_credit': inital_balance['credit'],
                'debit': ln['debit'],
                'credit': ln['credit'],
                'period_data': period_data
            })
        currency_id = self.env.company.currency_id
        return {'move_lines': move_lines, 'string': string, 'currency_id': currency_id, 'compare_period': compare_period}
    ...
    def get_html(self, dates='this_month', entry_type='all', journal_id=False, company_id=False, prev_period=False, last_period=False):
        ...
        # res['lines'] = self.env.ref('account_financial_reports_knk.report_trial_balance_document')._render({'data': vals})
        res['lines'] = self.env['ir.ui.view']._render_template('account_financial_reports_knk.report_trial_balance_document', {'data': vals})
        res['selected_date'] = vals['string']
        return res
